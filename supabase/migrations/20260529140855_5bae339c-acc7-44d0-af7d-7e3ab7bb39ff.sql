...
BEGIN
  ...
  -- Allow upgrade to finalizado from any non-terminal state (including previa_*)
  IF v_has_final AND v_current_status NOT IN ('finalizado', 'retificando', 'retificada', 'impedida', 'dispensada') THEN
    UPDATE irpf_cases SET status = 'finalizado', updated_at = now() WHERE id = p_case_id;
    RETURN;
  END IF;

  -- Protected states: do not auto-downgrade
  IF v_current_status IN ('impedida', 'dispensada', 'previa_enviada', 'previa_aprovada', 'retificando', 'retificada', 'finalizado') THEN RETURN; END IF;
...
BEGIN
  ...
  -- Allow upgrade to finalizado even from previa_* states
  IF v_has_irpf AND v_has_receipt AND v_sent_to_client
    ...
    UPDATE irpf_cases SET progress_percent = v_progress, status = 'finalizado' WHERE id = p_case_id;
    RETURN;
  END IF;
...
-- Repair Fernando's case
UPDATE irpf_cases SET status = 'finalizado', updated_at = now()
WHERE id = '6d36f04d-6d9e-48af-947e-916935462386';

INSERT INTO case_timeline (case_id, event_type, description, visible_to_client, created_by)
VALUES ('6d36f04d-6d9e-48af-947e-916935462386', 'Status alterado',
        'Status corrigido para Finalizado após envio da declaração final ao cliente (ajuste de trigger)',
        false, 'sistema');